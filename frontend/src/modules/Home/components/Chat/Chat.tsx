  ...
  return (
    <div className="flex h-full flex-col bg-gray-100">
      <div className="flex grow flex-col justify-end p-4">
        {!messages?.messages && <CallfaceMessage notIngrated />}
...
const CallfaceMessage = ({ notIngrated }: any) => {
  return (
    <>
      <div className="ml-auto flex items-center space-x-2">
        <p className="text-sm text-gray-900">Callface</p>
        <img
          src={AvatarCallface}
          alt="sales-photo"
      ...
      <div
        className={`bg-brand-primary my-2 ml-auto max-w-xs rounded-lg p-2 text-white`}
      >
        <p>{notIngrated ? 'Oi Antonio, como posso te ajudar hoje' : ''}</p>
      </div>
    </>
  );
};
